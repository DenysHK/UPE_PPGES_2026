...
"101325 Pa EXP (49)", reproduzir exatamente a escolha dos SETE
casos R1..R7 e criar uma nova aba chamada "SETE ESCOLHIDOS".
...
1) Universo de candidatos:
   ...
   - para R1..R6, restringir a escolha às composições intermediárias de CH4:
     0,50; 0,525; 0,55; 0,575; 0,60.
   - para R7, considerar todo o universo de composições intermediárias.

2) Faixas-alvo definidas a partir da SFT:
   - faixa de acoplamento (R1..R6):
       limite inferior = 1,80
       centro da faixa = 1,90
       limite superior = 2,00
   - pico ótimo da SFT (R7):
       centro do pico = 2,10

3) Regras para reproduzir exatamente R1..R7:
   - R1: para CH4 = 0,50, escolher o caso com H2/CO > 2,00
         mais próximo de 2,00  -> "âncora superior da faixa de acoplamento";
   ...
   - R5: para CH4 = 0,575, escolher o caso com H2/CO >= 1,80
         mais próximo de 1,80  -> "âncora inferior da faixa de acoplamento";
   - R6: para CH4 = 0,60, escolher o caso mais próximo de 1,90;
   - R7: em todo o universo de composições intermediárias (CH4: 0,50..0,60),
         escolher o caso com H2/CO mais próximo de 2,10
         -> "âncora do pico ótimo da SFT" (corrente mais favorável
            à produção de gasolina sintética C5–C11).

OBSERVAÇÃO IMPORTANTE
---------------------
R7 representa uma dimensão adicional de análise em relação a R1..R6.
Enquanto R1..R6 foram selecionados para cobrir sistematicamente a faixa
[1,80; 2,00] de H2/CO, R7 foi adicionado para representar a corrente da
RSB mais aderente ao pico ótimo identificado na SFT (~2,10), independente
de composição. Isso permite avaliar, na integração RSB–SFT, qual corrente
real da reforma melhor atende à condição de maior produção de C5–C11.

Na prática, R7 corresponde a CH4=0,60, T=1073 K, H2/CO≈2,117, e junto
com R1 (H2/CO≈2,052) forma o par de correntes mais promissoras para o
acoplamento, conforme discutido no Capítulo 4 da dissertação.
"""
...
OUTPUT_FILE = Path("/mnt/data/RSB_Results_com_SETE_ESCOLHIDOS.xlsx")

# Aba da qual os dados serão lidos.
SOURCE_SHEET = "101325 Pa EXP (49)"

# Nome da nova aba que será criada.
TARGET_SHEET = "SETE ESCOLHIDOS"

# Valores da faixa-alvo de acoplamento vindos da SFT (R1..R6).
H2CO_LOW    = 1.80
H2CO_CENTER = 1.90
H2CO_UP     = 2.00

# Pico ótimo da SFT identificado na análise (R7).
H2CO_SFT_PEAK = 2.10

# Composições intermediárias consideradas na seleção de R1..R6.
...
def safe_float(value):
    ...
    Retorna None se a célula estiver vazia ou for fórmula não resolvida.
    """
    if value is None:
        return None
...
# Carrega o workbook SEM data_only para ler valores numéricos diretamente
# das células (as fórmulas como H2/CO = H2/CO serão calculadas pelo Python
# a partir dos valores brutos de H2 e CO).
# NOTA: data_only=True retorna None para células com fórmulas em ambientes
# sem cache de cálculo (e.g. arquivos salvos fora do Excel). Por isso,
# calculamos H2/CO diretamente de row 17 (H2) e row 14 (CO).
wb_values = load_workbook(INPUT_FILE, data_only=False)
ws_values = wb_values[SOURCE_SHEET]

# Workbook separado para edição/escrita (mantém formatação original).
...
for col in range(3, ws_values.max_column + 1):
    ...
    h2_v = ws_values.cell(17, col).value   # H2 mol/s  (valor numérico direto)
    co_v = ws_values.cell(14, col).value   # CO mol/s  (valor numérico direto)

    # Se não houver composição ou fluxos, a coluna não é um caso válido.
    if ch4 is None or h2_v is None or co_v is None:
        continue

    try:
        h2_f  = float(h2_v)
        co_f  = float(co_v)
        h2co  = h2_f / co_f          # H2/CO calculado diretamente
        ch4_f = float(ch4)
...
# ======================================================================
# 4) FILTRO INICIAL: SOMENTE COMPOSIÇÕES INTERMEDIÁRIAS (R1..R6 e R7)
# ======================================================================

df = df_all[df_all["CH4_frac"].isin(INTERMEDIATE_CH4)].copy()

# ======================================================================
# 5) APLICAÇÃO DOS CRITÉRIOS EXATOS PARA R1..R7
# ======================================================================

# R1 = âncora superior da faixa de acoplamento:
...
# R5 = âncora inferior da faixa de acoplamento:
...
# R7 = âncora do pico ótimo da SFT:
# em todo o universo de composições intermediárias (CH4: 0,50..0,60),
# escolher o caso com H2/CO mais próximo de 2,10 (pico de produção de
# gasolina sintética C5–C11 identificado na análise da SFT).
# Resultado esperado: CH4=0,60, T=1073 K, H2/CO≈2,117 (coluna AM).
# Esse caso complementa R1 (H2/CO≈2,052) ao cobrir a região acima de 2,10,
# sendo a corrente da RSB mais próxima do pico ótimo da Fischer-Tropsch.
R7 = pick_closest(
    df,
    H2CO_SFT_PEAK,
    "R7"
...
R7["Case"] = "R7"
R7["Rule"] = "CH4=0,50..0,60; menor |H2/CO-2,10|  (pico ótimo SFT)"

selected = pd.DataFrame([R1, R2, R3, R4, R5, R6, R7])
...
# ======================================================================
# 6) CRIAÇÃO / RECRIAÇÃO DA ABA "SETE ESCOLHIDOS"
# ======================================================================
...
fill_r7      = PatternFill("solid", fgColor="FFF2CC")   # destaque amarelo para R7
...
# Bloco de título
# ----------------------------------------------------------------------
ws_new["A1"] = "SETE ESCOLHIDOS PARA ACOPLAMENTO RSB–SFT"
...
# ----------------------------------------------------------------------
# Bloco explicando os critérios
# ----------------------------------------------------------------------
criteria_lines = [
    "Origem dos dados: aba '101325 Pa EXP (49)' de RSB_Results.xlsx.",
    "Filtro inicial (R1–R6): somente composições intermediárias CH4 = 0,50; 0,525; 0,55; 0,575; 0,60.",
    "Faixa-alvo de acoplamento (R1–R6): limite inferior = 1,80; centro = 1,90; limite superior = 2,00.",
    "R1: CH4=0,50 e H2/CO>2,00; escolher o mais próximo de 2,00 (âncora superior da faixa de acoplamento).",
    ...
    "R5: CH4=0,575 e H2/CO>=1,80; escolher o mais próximo de 1,80 (âncora inferior da faixa de acoplamento).",
    "R6: CH4=0,60; escolher o mais próximo de 1,90.",
    "R7: todo o universo CH4=0,50..0,60; escolher o mais próximo de 2,10 (âncora do pico ótimo da SFT).",
    "Justificativa do R7: a análise da SFT identificou H2/CO ≈ 2,10 como o pico de produção de gasolina",
    "sintética C5–C11. R7 representa a corrente da RSB mais aderente a esse pico, complementando",
    "R1 (H2/CO > 2,00) e cobrindo a faixa acima de 2,10. R1 e R7 são as correntes mais promissoras",
    "para o acoplamento, conforme demonstrado na análise integrada RSB–SFT (Capítulo 4).",
]
...
ws_new.cell(note_row, 1).value = (
    "Observação: R1..R6 cobrem sistematicamente a faixa de acoplamento [1,80; 2,00], "
    "incluindo âncora superior (R1), casos centrais (R2, R3, R4, R6) e âncora inferior (R5). "
    "R7 (destacado em amarelo) é adicionado como âncora do pico ótimo da SFT (H2/CO≈2,10), "
    "representando a corrente da RSB mais favorável à produção de gasolina sintética C5–C11."
)
...
print(f"\nResumo dos {len(selected)} casos selecionados:")
print(selected[["Case", "Column", "Temperature_K", "CH4_frac", "H2_CO",
                "criterion_target", "criterion_distance"]].to_string(index=False))
